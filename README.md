...
BRAT 请指定 `1.0.0-beta.6`，本次为 prerelease，正式版仍为 0.5.7。建议在 Windows、iOS、Android 分别验证：横向列表、筛选和搜索、库存归零显示待补货、不再买的编辑与同步、库存零下限，以及现有新增编辑和照片操作。
...
首页统一为单列横向紧凑卡片，沿用最近/资料的背景。名称单行省略，单件集中显示购买总价、拥有天数、日均价及购买日期，未填价格时不显示金额；库存显示剩余数量与轻量加减控件。筛选为全部/使用中/库存/待补货/历史，搜索继续覆盖历史物品的名称和备注。库存归零且仍计划补货时自动显示待补货；编辑库存可选择“不再买”，它与已退役单件一起进入历史。旧物品无需迁移，缺少新可选字段时按继续补货处理。
