...
export class DoudouView extends ItemView {
  ...
  private async switchPage(page: DoudouPage): Promise<void> { if (this.showingRecord) this.closeRecord(); this.activePage = page; const all = page === "all"; if (all) this.libraryPage.deactivate(); else this.allPage.deactivate(); this.allContainerEl.toggleClass("doudou-is-hidden", !all); this.libraryContainerEl.toggleClass("doudou-is-hidden", all); this.allTabEl.toggleClass("doudou-is-selected", all); this.libraryTabEl.toggleClass("doudou-is-selected", !all); this.dependencies.repository.invalidateCache(); if (all) await this.allPage.refresh(false); else await this.libraryPage.activate(); }
