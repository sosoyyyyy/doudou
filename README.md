...
在小物库点击「导入已有物品」，选择本地导入 JSON，逐项核对名称、日期、价格、状态和备注，最后点击「确认导入」。预览或取消不会写入物品数据。相同导入编号只写入一次，重复导入会跳过；删除已导入物品也不会导致下一次导入自动恢复它。导入不覆盖已有物品。

导入文件格式为 `{"format":"doudou-items-import-v1","items":[...]}`。每项包含唯一 `key`、`name`，以及可选 `kind`（single/stock）、`purchased`（YYYY-MM-DD）、`price`、`status`（active/retired）、`retired`、`quantity` 和 `notes`。不导入公式、派生天数、日均价或外部照片路径。真实数据导入文件不提交到 Git，也不随 Release 分发。

### 模块与同步

`src/items/model.ts` 定义独立类型、校验和日期计算；`ItemRepository.ts` 通过 Vault API 读写带版本号的 JSON；`ItemService.ts` 提供保存、删除、调整库存和批量导入方法；`ItemsPage.ts` 负责页面和确认流程。后续批量工具可调用 service，本版本未接入 AI。

单个插件实例的写入串行执行，更新前读取最新 JSON；旧编辑会被版本检查拦截。跨设备同步仍由用户的 Vault 同步工具完成，不提供多设备同时离线修改同一 JSON 的自动合并。数据损坏或版本不兼容时停止写入，不用空列表覆盖原文件。

BRAT 请指定 `1.0.0-beta.1`，本次为 prerelease，正式版仍为 0.5.7。建议在 Windows、iOS、Android 分别验证：导航/搜索、单件新增编辑退役、库存连点和零下限、照片取消/保存/删除、导入取消/确认/重复导入，以及同步后重新打开。
